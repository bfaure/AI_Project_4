...
def plot(matrix,weights=None,title="Prediction Matrix",save_plot=False):
	...
	if len(matrix[0])==3: # if 1D inputs, excluding bias and ys 
		...
		if save_plot:
			plt.save_fig(os.path.join("plots",title+".png"),bbox_inches='tight',dpi=200)
		else:
			plt.show()
		return
...
# each matrix row: up to last row = inputs, last row = y (classification)
def train_weights(matrix,weights,nb_epoch=10,l_rate=1.00,do_plot=False,stop_early=True,save=True):
	if save:
		log_file = open("classifier_log.tsv","w")
		log_file.write("accuracy\t")
		for i,w in enumerate(weights):
			log_file.write("w%d\t"%(i))
		log_file.write("\n")

	last_accuracy=accuracy(matrix,weights)
	...
	for epoch in range(nb_epoch):
		...
		if cur_acc<last_accuracy and stop_early:
			print("Model getting worse, ending training.")
			weights=last_weights
			break
		else:
			last_accuracy=cur_acc
			last_weights=weights

		if save: 
			log_file.write("%0.10f\t"%cur_acc)
			for w in weights:
				log_file.write("%0.10f\t"%(w))
			log_file.write("\n")

		if cur_acc==1.0 and stop_early: 
			print("Model at 100%% accuracy, ending training.")
			break 
		if do_plot: plot(matrix,weights,title="Epoch_%d"%epoch)		
...
def main():
	...
	train_weights(matrix,weights=weights,nb_epoch=nb_epoch,l_rate=l_rate,do_plot=plot_each_epoch,stop_early=stop_early,save=save)
